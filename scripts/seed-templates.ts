...
type ContentTier = "standard" | "premium" | "social";
...
async function seedTemplates() {
  ...
  const { error: insertError } = await supabase.from("templates").insert(templates);
  if (insertError) {
    throw insertError;
  }
  ...
  console.log(`Seeded ${templates.length} templates successfully. Current templates count: ${count ?? "unknown"}.`);
}
