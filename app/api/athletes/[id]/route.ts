...
export async function PATCH(request: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    const { data, error } = await supabase.from("athletes").update(body).eq("id", params.id).select("*").single();
...
export async function DELETE(request: NextRequest, { params }: { params: { id: string } }) {
  try {
    await requireAuthenticatedOperator(request);
    const supabase = getAdminSupabase();
    await supabase.from("assets").delete().eq("owner_type", "athlete").eq("owner_id", params.id);
